...
class discrete_BCQ(object):
    def __init__(
        ...
        self.Q = Conv_Q(state_dim[0], num_actions).to(
            self.device) if is_atari else FC_Q(state_dim, num_actions).to(self.device)
    ...
    def train(self, replay_buffer):
        ...
        with torch.no_grad():
            ...
            target_Q = reward + done * self.discount * \
                q.gather(1, next_action).reshape(-1, 1)
