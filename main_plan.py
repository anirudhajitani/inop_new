...
if __name__ == "__main__":
    ...
    num_actions = 2
    gamma = 0.95
    env = OffloadEnv(True, args.lambd, args.offload_cost, args.overload_cost, args.holding_cost, args.reward, args.N, args.seed, args.env_name, args.folder)
    ...
    for i in range(loop_range):
        ...
        #print ("LOOP ", i)
        #for j in range(0, 10):
        #lambd = np.load(f"./{args.folder}/buffers/lambda_{3}_{j}.npy")
        #N = np.load(f"./{args.folder}/buffers/N_{3}_{j}.npy")
        #    model_name = f"./{args.folder}/models/model_{args.algo}_{j}_{i}"
            #print ("Lambd N i ", lambd[i], N[i])
        env.set_N(int(N[i]), list(lambd[i]))
        if args.algo == 0:
            if os.path.isfile(f"./policies/policy_plan_lambd_{sum(lambd[i])}.npy"):
                policy = np.load(f"./policies/policy_plan_lambd_{sum(lambd[i])}.npy")
        ...
        for k in range(100):
            ...
            if args.algo == 2:
                ...
                if os.path.isfile(f"./policies/policy_plan_lambd_{est_lam}.npy"):
                    ...
                    pol = PlanPolicy(N[i], est_lam_vec, args.overload_cost, args.offload_cost, args.holding_cost, args.reward, gamma=gamma, prob=resources_prob)
                    policy = pol.compute_policy(save=True)
                policy = list(policy)
            reward_traj = []
            dis_reward = 0.0
