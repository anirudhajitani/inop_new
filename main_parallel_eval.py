...
def run_eval_iter(args, loop_start, loop_end, model, env, N, lambd):
    testing_eval_med = []
    for i in range(loop_start, loop_end):
        ...
        quartiles = np.percentile(avg_reward_run, [25, 50, 75])
        print ("quartiles ", quartiles)
        testing_eval_med.append(quartiles)
        #testing_eval_std.append(avg_std)
        np.save(f"./{args.folder}/results/median_{loop_start}_{loop_end}_{setting}", testing_eval_med)
...
if __name__ == "__main__":
    ...
    state_dim = 2
    num_actions = 2
    env = OffloadEnv(True, args.lambd, args.offload_cost, args.overload_cost, args.holding_cost, args.reward, args.N, args.seed, args.env_name, args.folder, add_stats=False)
    ...
    pool = mp.Pool(args.step)
    result_objects = [pool.apply_async(run_eval_iter, args=(args, i*10, (i+1)*10, env, N, lambd)) for i in range(args.start_iter, args.start_iter+args.step)]
    pool.close()
    pool.join()
