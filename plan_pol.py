...
class PlanPolicy():
    def __init__(self, N, lambd, overload, offload, holding, reward, gamma=0.95):
        ...
        self.V_alt = np.zeros(self.N_STATES)
        self.policy = [1 for s in range(self.N_STATES)]
        self.old_policy = [1 for s in range(self.N_STATES)]
        ...
        while is_policy_stable is False:
            ...
            while is_value_changed:
                ...
                for s in range(self.N_STATES):
                    ...
                    self.V_alt[s] = sum([self.P[s, self.old_policy[s], s1] * (self.R[s, self.old_policy[s] ,s1] + self.gamma*self.V_alt[s1]) for s1 in range(self.N_STATES)])
            ...
            for s in range(self.N_STATES):
                ...
                if old_action != self.policy[s]:
                    ...
                    #print ("Diff ", np.array(self.V), np.array(self.V_alt))
                    is_policy_stable = False
    ...
    def set_policy(self):
        T = [ 18.10266556,  17.87829124,  16.82954507,  15.25107209,
             9.52432038,   9.17277454,   6.87608255,   6.56708564,
             6.16812193,   6.12824105,   5.96961645,   5.72258153,
             5.64404315,   5.62790141,   5.52036458,   4.56112876,
             4.38465399,   2.7171501 ,   1.53149777,   0.48978683,   0.12719765]
        
        for i in range(len(T)):
            for j in range(21):
                if round(T[i]) >= j:
                    try:
                        self.old_policy[i*21 + j] = 0
                    except:
                        print (i*21+j)
        print (self.old_policy)
    ...
    def compute_policy(self, plot=False, decay_rew=False, save=False, name=None):
        ...
        self.print_policy()
        if name is None:
            name = f"./policies/policy_plan_lambd_{sum(self.lambd)}.npy"
        if save == True:
    ...
    def print_policy(self):
        ...
        print (self.V)
        print (self.V_alt)
        print (np.array(self.V) - np.array(self.V_alt))
        np.save(f"./value_fn/value_fn_{sum(self.lambd)}.npy", self.V)
