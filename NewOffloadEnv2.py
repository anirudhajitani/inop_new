...
class OffloadEnv(gym.Env):
    ...
    def __init__(self, eval_, lambd, offload, overload, holding, reward, N, seed, env_name=None, folder=None):
        ...
        if self.eval_ == True:
            ...
            if self.seed_count % 40 == 39:
                ...
                if self.outer_seed_count % 10 == 0: 
                    ...
                    self.overload_run = []
                    self.offload_run = []
                    np.save(f"./{self.folder}/results/overload_med_{self.env_name}.npy", self.med_overload) 
                    #np.save(f"./{self.folder}/results/overload_std_{self.env_name}.npy", self.std_overload) 
                    np.save(f"./{self.folder}/results/offload_med_{self.env_name}.npy", self.med_offload) 
            ...
            self.seed_count = (self.seed_count + 1) % 40
