...
class structured_learning(object):
    def __init__(
        ...
        print ("SLOW LR, FAST LR, FIXED STATE ", self.step_size_slow, self.step_size_fast, self.fixed_state)
    ...
    def adam_lr(self, state_val, g, step_size, t):
        ...
        state_val = state_val - step_size * m_hat / (np.sqrt(v_hat) + self.epsilon)
        return state_val
    ...
    def train(self, state, action, reward, next_state, eval_freq, env_name, folder, j):
        ...
        #g = reward + self.discount * min(self.val_fn[en_next_state, 0], self.val_fn[en_next_state, 1]) - self.val_fn[self.fixed_state, action] 
        #g = reward + self.discount * min(self.val_fn[en_next_state, 0], self.val_fn[en_next_state, 1]) 
        val_update = self.adam_lr(self.val_fn[en_state, action], g, self.step_size_fast, self.state_counts[en_state, action])
        #if g > 0:
        ...
        if debug:
            print("Value fn after, Action, g", en_state, self.val_fn[en_state], action, g)
        ...
        if debug:
            print("Thres before : , mul", state[1], self.req_thres[int(state[0])], mul)
        ...
        self.req_thres[int(state[0])] = min(max(self.adam_lr(self.req_thres[int(state[0])], g, self.step_size_slow, self.state_counts[en_state, action]), 0.0), 20.0)
        if debug:
            print ("Thres after ", self.req_thres[int(state[0])], " Action ", action, " g = ", g)
